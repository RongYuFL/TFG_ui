...
```mermaid
graph TD
    A[前端: 用户点击“开始录音”] --> B(前端: 用户说话...);
    B --> C[前端: 用户点击“停止录音”];
    C --> D[JS: 上传音频 Blob至 /save_audio];
    D --> E[Flask: app.py @ /save_audio];
    E --> F[Pydub: 修复并保存为标准 aud.wav];
    
    G[前端: 用户点击“开始对话”] --> H[JS: 提交表单至 /chat_system];
    H --> I[Flask: app.py @ /chat_system];
    I --> J[调用 backend/chat_engine.py];
    
    J --> K[Engine: 运行 china_pipeline.py];
    K --> L[ASR/LLM: aud.wav -> latest_ai_response.txt];
    
    L --> M[Engine: 切分长文本];
    M --> N[Engine: 循环调用 RVC.py];
    N --> O[RVC: text_chunk -> chunk_N.wav];
    
    O --> P[Engine: 使用 pydub 拼接所有 chunk_N.wav];
    P --> Q[保存为 static/audios/cloned_output.wav];
    
    Q --> R[Flask: 返回 JSON 路径];
    R --> S[JS: 接收路径, 更新 <video> 标签并播放];
```
